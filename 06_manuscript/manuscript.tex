...
\caption{\textbf{Virtual screening performance comparison.} (A)~ROC curves for naive (blue) and skill-guided (orange) protocols with 95\% bootstrap confidence bands. Inset: early enrichment region (FPR 0--5\%). (B)~BEDROC($\alpha$=20), EF$_{1\%}$, and EF$_{5\%}$ with bootstrapped error bars. (C)~Violin plots of docking score distributions for actives (red) vs.\ decoys (grey); horizontal lines indicate medians. Mann--Whitney $U$ $p$-values annotated.}
\label{fig:main}
\end{figure*}
